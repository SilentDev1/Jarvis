...
if [ -f run/local-device-gateway.pid ] && kill -0 "$(cat run/local-device-gateway.pid)" 2>/dev/null; then echo "LOCAL AIPI GATEWAY RUNNING pid=$(cat run/local-device-gateway.pid)"; else echo "LOCAL AIPI GATEWAY STOPPED"; exit 1; fi
